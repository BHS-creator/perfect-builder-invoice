...
function calc(){let sub=0;items.querySelectorAll('tr').forEach(r=>{let q=+r.querySelector('.qty').value||0,p=+r.querySelector('.price').value||0,t=q*p;sub+=t;r.querySelector('.rowtotal').textContent=t.toFixed(2);r.classList.toggle('empty',!r.querySelector('.item').value.trim()&&!+r.querySelector('.price').value)});let rate=+document.getElementById('vat').value||0,v=sub*rate/100;document.getElementById('subtotal').textContent=sub.toFixed(2);document.getElementById('vatLabel').textContent=rate+'%';document.getElementById('vatAmount').textContent=v.toFixed(2);document.getElementById('total').textContent=(sub+v).toFixed(2)}
function newInvoice(){if(!confirm('Start a new invoice?'))return;items.innerHTML='';document.querySelectorAll('.customer input,.project input').forEach(x=>x.value='');document.querySelectorAll('.editable').forEach(x=>x.innerHTML='');document.querySelector('.inline').textContent='INV-0001';date.value=new Date().toISOString().slice(0,10);document.getElementById('vat').value=0;projectTotal.value='';receivedPayment.value='';calcPayment();addItem()}const projectTotal=document.getElementById('projectTotal');
const receivedPayment=document.getElementById('receivedPayment');
function calcPayment(){
  const total=+projectTotal.value||0, received=+receivedPayment.value||0;
  document.getElementById('remainingPayment').textContent=Math.max(0,total-received).toFixed(2);
}
